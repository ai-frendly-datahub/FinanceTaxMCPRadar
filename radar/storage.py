...
class RadarStorage:
    ...
    def _ensure_tables(self) -> None:
        ...
        if "ontology_json" not in columns:
            _ = self.conn.execute("ALTER TABLE articles ADD COLUMN ontology_json TEXT")
    ...
    def upsert_articles(self, articles: Iterable[Article]) -> None:
        ...
        try:
            ...
            _ = self.conn.executemany(
                """
                INSERT INTO articles (category, source, title, link, summary, published, collected_at, entities_json, ontology_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
